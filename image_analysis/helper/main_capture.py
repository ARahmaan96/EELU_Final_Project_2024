...
print(os.getcwd())
sound = mixer.Sound('alarm.wav')  # please open your eyes sound

face_cascade = cv2.CascadeClassifier(
    'DDDS_CNN/haar cascade files/haarcascade_frontalface_alt.xml')
leye_cascade = cv2.CascadeClassifier(
    'DDDS_CNN/haar cascade files/haarcascade_lefteye_2splits.xml')
reye_cascade = cv2.CascadeClassifier(
    'DDDS_CNN/haar cascade files/haarcascade_righteye_2splits.xml')
...
while True:
    ...
    if rpred[0] == 0 or lpred[0] == 0:
        ...
        if time.time() - close_time >= alarm_delay:
            cv2.putText(frame, "Sleepy!", (10, height-20), font,
        ...
        if alarm_on:
            sound.stop()
            alarm_on = False
        cv2.putText(frame, "Alert", (10, height-20), font,
                    1, (255, 255, 255), 1, cv2.LINE_AA)
